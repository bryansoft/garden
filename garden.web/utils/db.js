...
db.save('_design/measurements', {
  views: {
    ...
    byHour: {
      // map
      map: function(doc){
        emit(Math.round("test", parseFloat(doc.moisture)) // in place of doc.info.size, you'd put whatever
...
module.exports = {
  ...
  latestMeasurementsByHour: function(callback){
    log.info("Querying measurements by endkey");
    db.view("measurements/byHour", {limit:1000, descending:true}, function(err, results){
